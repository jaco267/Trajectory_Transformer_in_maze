...
def sample(model, x,mode=None,  temperature=1.0, topk=None, cdf=None,**forward_kwargs):
    '''
    ...
    # if mode == 'action':
    #     print(raw_probs[0,:5],raw_probs[0,:5].sum(),'action')
        # breakpoint()
...
@torch.no_grad()
def sample_n(model, x, N,mode=None,  **sample_kwargs):
    batch_size = len(x)
